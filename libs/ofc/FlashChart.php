<?php
	...
	class FlashChart
	{
		...
		public function getChart( $width, $height, $url, $use_swfobject=true, $base='' )
		{
			//
			// return the HTML as a string
			//
			return $this->prepareChart( $width, $height, $url, $use_swfobject, $base );
		}

		public function showChart( $width, $height, $url, $use_swfobject=true, $base='' )
		{
			//
			// stream the HTML into the page
			//
			echo $this->prepareChart( $width, $height, $url, $use_swfobject, $base );
		}

		private function prepareChart( $width, $height, $url, $use_swfobject, $base )
		{
			//
			...
			if( $use_swfobject )
			{
				...
				$out[] = 'var so = new SWFObject("'. $base .'flash/open-flash-chart.swf", "'. $obj_id .'", "'. $width . '", "' . $height . '", "9", "#FFFFFF");';
			...
			$out[] = '<param name="movie" value="'. $base .'flash/open-flash-chart.swf?data='. $url .'" />';
			$out[] = '<param name="quality" value="high" />';
			$out[] = '<param name="bgcolor" value="#FFFFFF" />';
			$out[] = '<embed src="'. $base .'flash/open-flash-chart.swf?data=' . $url .'" quality="high" bgcolor="#FFFFFF" width="'. $width .'" height="'. $height .'" name="'. $obj_id .'" align="middle" allowScriptAccess="sameDomain" ';
